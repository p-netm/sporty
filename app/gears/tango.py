...
def save_flagged(diction, *vars):
	""":parameter dict"""
	country_obj, league_obj, home_team_obj, away_team_obj = pre_save(diction)
	flag_obj = Flagged.query.filter(Match.date == diction['date']).filter(Match.time == diction['time']).filter(Match.team_two == away_team_obj.id).filter(Match.team_one == home_team_obj.id).first()
	if flag_obj is None:
